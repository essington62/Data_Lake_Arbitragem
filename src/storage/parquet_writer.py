...
from src.normalizers.transforms import to_dataframe, order_book_to_dataframe
from src.normalizers.schema import DEDUP_KEYS, ORDER_BOOK_DEDUP_KEYS
...
def write_order_book(records: list[dict], base_path: str = "data/normalized") -> None:
    if not records:
        logger.warning("write_order_book called with empty records — skipping")
        return

    df = order_book_to_dataframe(records)
    df["_date"] = df["timestamp"].dt.date

    written = 0
    for (exchange, date), group in df.groupby(["exchange", "_date"]):
        group = group.drop(columns=["_date"])
        part_dir = Path(base_path) / "order_book" / f"exchange={exchange}" / f"date={date}"
        part_dir.mkdir(parents=True, exist_ok=True)
        outfile = part_dir / "data.parquet"
        ...
        if outfile.exists():
            existing = pd.read_parquet(outfile)
            group = pd.concat([existing, group], ignore_index=True).drop_duplicates(
                subset=ORDER_BOOK_DEDUP_KEYS, keep="last"
            )
    ...
    logger.info(f"write_order_book: {written} total rows across {df['exchange'].nunique()} exchanges")
